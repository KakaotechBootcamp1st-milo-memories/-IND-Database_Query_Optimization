...
## 쿼리 성능 비교
[기존 쿼리 성능 측정](#기존-쿼리-성능-측정)에서 측정했던 쿼리의 소요 시간은 2.66s였으나 쿼리문 최적화, 인덱스 적용 후 [성능 재측정](#성능-재측정-복합-인덱스-추가-전과-복합-인덱스-추가-후-성능-비교) 결과는 0.58s로 약 5배 최적화가 된 것을 확인할 수 있었습니다.

## 인덱스 설계 문서
[인덱스 사용 이유](#인덱스-사용-이유)부터 설계하였습니다.
