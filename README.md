...
### 1. 병목 현상
- **쿼리 캐시의 동기화 문제**: 쿼리 캐시는 쓰기 작업이 발생할 때마다 무효화됩니다. 예를 들어, 테이블에 데이터가 삽입, 삭제, 수정되면 해당 테이블에 대한 모든 쿼리 캐시가 무효화됩니다.
...
### 3. 효율성의 한계
- **동일한 쿼리만 캐시 가능**: 쿼리 캐시는 정확히 동일한 쿼리에 대해서만 결과를 캐시합니다. 이로 인해 쿼리 캐시의 효과는 매우 제한적입니다.
